...
# Para esperar un tiempo X la respuesta pong (ECHO_REPLY)
from select import select
# Para generar el ID de la cabecera ICMP y generar los datos de la cabecera ICMP
...
# Cantidad de veces predeterminada de envios ECHO_REQUEST (infinito)
DEFAULT_COUNT = 0
# Cantidad de tiempo de espera de intervalo entre envio de cada ECHO_REQUEST
DEFAULT_INTERVAL = 1
# Cantidad de tiempo de espera de la respuesta ECHO_REQUEST (ECHO_REPLY)
DEFAULT_TIMEOUT = 2
...
def calcular_checksum(DATOS):
    ...
    cheksum = int(format(cheksum, "016b").translate(
        {ord('0'): '1', ord('1'): '0'}), 2) & 65535
...
def desempaquetar_ip(PAQUETE, FROM=0, TO=20, CHARACTER=""):
    ...
    # Differentiate Services Codepoint - Differentiate Servic Field
    dtgr_ip["dsc" + CHARACTER] = ip_header[1] >> 2
    # Explicit Services Codepoint - Differentiate Servic Field
    dtgr_ip["enc" + CHARACTER] = ip_header[1] & 15
    # Total lenght
    dtgr_ip["total length" + CHARACTER] = ip_header[2]
    dtgr_ip["id" + CHARACTER] = ip_header[3]                   # Identification
    # Reserver bit Flags # obtener el primer bit y desplazamiento a la derecha 15 bits
    dtgr_ip["rsb" + CHARACTER] = ip_header[4] >> 15
    # 16384
    # Don't fragments # obtener el segundo bit y desplazamiento a la derecha 14 bits
    dtgr_ip["dtf" + CHARACTER] = (ip_header[4] & 0x4000) >> 14
    # 8192
    # More fragments # obtener el primer bit y desplazamiento a la derecha 13 bits
    dtgr_ip["mrf" + CHARACTER] = (ip_header[4] & 0x2000) >> 13
    # 8191
    # Fragment offset
    dtgr_ip["frag offset" + CHARACTER] = ip_header[4] & 0x1fff
    ...
    # Source address
    dtgr_ip["source address" + CHARACTER] = socket.inet_ntoa(ip_header[8])
    # Destination address
    dtgr_ip["destination address" + CHARACTER] = socket.inet_ntoa(ip_header[9])
...
def desempaquetar_icmp(PAQUETE):
    ...
    # Si no es tipo 5 (Regirigir ruta) el paquete tiene ID y sequencia
    if dtgr_icmp["type"] != 5:
        ...
        dtgr_icmp["gateway address"] = socket.inet_ntoa(
            unpack("!4s", PAQUETE[24:28])[0])  # Gateway address
...
def recibir_pong(SOCK, DESTINATION_ADDRESS, ID, SEQ, TIMEOUT, DEBUG):
    ...
    while True:
        ...
        if datagrama_icmp["type"] == 5 and datagrama_icmp["id_"] == ID and datagrama_icmp["seq_"] == SEQ:
            print("Desde %s: icmp_seq=%d Redirigir ruta (Nuevo próximo salto: %s)" % (
                datagrama_ip["source address"], datagrama_icmp["seq_"], datagrama_icmp["gateway address"]))
            if DEBUG:
...
def ping(DESTINATION: str, SIZE_DATA: int = DEFAULT_SIZE_DATA, COUNT: int = DEFAULT_COUNT, INTERVAL: float = DEFAULT_INTERVAL, TIMEOUT: float = DEFAULT_INTERVAL, DEBUG=False):
    ...
    try:
        ...
        print("PING %s (%s) %d(%d) bytes de datos." %
              (DESTINATION, ip_address, SIZE_DATA, SIZE_DATA+28))
        ...
        while True:
            ...
            if time_send != None:
                ...
                if datos[0]:
                    ...
                    tiempo_avg += rtt
                    print("%d bytes desde %s: icmp_seq=%d ttl=%d tiempo=%.2f ms" % (
                        datos[1]+8, alias_address, datos[2], datos[3], rtt))
                else:
                    if datos[1] != None:
                        print("Desde %s icmp_seq=%d %s" % (
                            datos[1], datos[2], convertir_tipo_y_codigo_a_texto(datos[3], datos[4])))
                        paquete_error += 1
    ...
    if transmitidos > 0:
        ...
        print("%d paquetes transmitidos, %d paquetes recibidos, %.2f%% paquetes perdidos, %stiempo %0.2fs" % (
            transmitidos, recibidos, (1-recibidos/transmitidos)*100, show_error, tiempo_avg))

        if tiempo_maximo != -1:
            print("rtt min/avg/max/ = %.2f/%.2f/%.2f ms" %
                  (tiempo_minimo, tiempo_avg / recibidos, tiempo_maximo))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--debug',   action='store_true',
                        help='Imprimir información de los paquetes')
    parser.add_argument('-c', '--count',   type=uint,    default=DEFAULT_COUNT,
                        help='Especifica la cantidad de veces que se debe enviar la solicitud de ping (ECHO_REQUEST)')
    parser.add_argument('-t', '--timeout', type=ufloat,  default=DEFAULT_TIMEOUT,
                        help='Especifica el tiempo de espera de la respuesta de la solicitud de ping en segundos (ECHO_REPLY). El valor predeterminado es %(default)s segundos')
    parser.add_argument('-s', '--size',    type=uintlimite, default=DEFAULT_SIZE_DATA,
                        help="Especifica el número de bytes de datos que se enviarán. El valor predeterminado es %d, que se traduce en %d bytes de datos ICMP más 8 bytes de datos del encabezado ICMP." % (DEFAULT_SIZE_DATA, DEFAULT_SIZE_DATA+8))
    parser.add_argument('-i', '--interval', type=ufloat,  default=DEFAULT_INTERVAL,
                        help='Especifica el tiempo de espera de intervalo entre el envío de cada paquete en segundos. El valor predeterminado es %(default)s segundo.')
    parser.add_argument(
        'destino', type=str, help="Especifica la dirección IPv4 A.B.C.D o el nombre a resolver www.rdc.com")

    args = parser.parse_args()

    ping(args.destino, args.size, args.count,
         args.interval, args.timeout, args.debug)
